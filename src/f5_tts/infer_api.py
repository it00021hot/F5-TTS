...
from fastapi import FastAPI, HTTPException, Response, Query, File, UploadFile, Form

from fastapi.responses import StreamingResponse

from pydantic import BaseModel

from infer.utils_infer import infer_batch_process, preprocess_ref_audio_text, load_vocoder, load_model, infer_process
...
class TTSStreamingProcessor:
    ...
    def generate_audio(self, text, ref_text, audio_file):
        """Generate audio for the given text and return it as a WAV file."""
       # 进行预处理
        ref_audio, ref_text = preprocess_ref_audio_text(audio_file, ref_text)
    ...
    def train(self, audio):
        ...
        _, ref_text = preprocess_ref_audio_text(audio, "")
...
async def text_to_speech(
    ...
    try:
        ...
        # 处理上传的参考音频
        audio_buffer = processor.generate_audio(text, ref_text, io.BytesIO(await audio.read()))
        chunk_size = 1024
...
@app.post("/train")
async def train(audio: UploadFile = File(...)):
    """
    ...
    try:
        ...
        # 使用 processor 的预处理功能解析文本

        text = processor.train(audio_buffer)

        return {"filename": audio.filename, "transcribed_text": text}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=str(cached_path("hf://SWivid/F5-TTS/F5TTS_v1_Base/model_1250000.safetensors")),
